	...
	if(!(lineIntersects [eyePos player, eyePos _x, player, _x]) && {!isNil {_x getVariable "realname"}}) then {
		...
		if(count _sPos > 1 && {_distance < 15}) then {
			_text = switch (true) do {
				...
				case (!isNil {(_x getVariable "rank")}): {format["<t color='#0000FF' size='1.5'><img image='%1' size='1.5'></img> %2 %3</t>",switch ((_x getVariable "rank")) do {
				...
				case ((!isNil {_x getVariable "name"} && playerSide == independent)): {format["<t color='#FF0000'><img image='a3\ui_f\data\map\MapControl\hospital_ca.paa' size='1.5'></img>Medic %1</t>",_x getVariable ["name","Unbekannter Spieler"]]};
				case ((!isNil {_x getVariable "name"} && playerSide == east)): {format["<t color='#FFFF00'>ADAC %1</t>",_x getVariable ["name","Unbekannter Spieler"]]};
				default {
